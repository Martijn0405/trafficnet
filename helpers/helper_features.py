...
def get_features(index, img, bbox, output_path, use_shi=False):
    ...
    for i in range(n_object):
        ...
        ax.add_patch(patch)
    os.makedirs(output_path + "/features", exist_ok=True)
    plt.savefig(output_path + "/features/features_" + str(index) + ".png", dpi=300, bbox_inches='tight')
    plt.close()
